...
<form class="needs-validation" novalidate>
  <div class="form-row">
   ...
   <div class="col-md-4 mb-3">
    ...
    <button type="submit" @click.prevent="createdep" class="btn btn-black">Criar</button>
...
async created(){
const res = await httpDep.listDepartment();
this.listDepartment = res.data.list
},
data(){
  return {
    nome : "",
...
methods:{
  ...
  this.$swal.fire({
  title: 'Tem certeza que deseja criar novo usuario?',
  icon: 'warning',
  ...
  if (result.isConfirmed) {
   ...
   const res = await http.createCollaborator(data);
   console.log(res);
   if(res.status == 201){
         Toast.fire({
          icon: "success",
